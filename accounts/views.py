...
def login(request):
  if request.method == 'POST':
    ...
    if user is not None:
      ...
      # http://127.0.0.1:8000/account/login/?next=/cart/checkout/
...
def forgotPassword(request):
  if request.method == 'POST':
    ...
    if Account.objects.filter(email=email).exists():
      ...
      messages.success(request, 'Un correo electrónico fue enviado a tu bandeja de entrada para recuperar tu contraseña')
      return redirect('login')
    else:
...
def resetPassword(request):
  if request.method == 'POST':
    ...
    if password == confirm_password:
      ...
      user.save()
      messages.success(request, 'Tu contraseña se restableció correctamente')
      return redirect('login')
    else:
...
def edit_profile(request):
  ...
  if request.method == 'POST':
    ...
    if user_form.is_valid() and profile_form.is_valid():
      user_form.save()
      profile_form.save()
      messages.success(request, 'Tu información se guardó con éxito')
